# STMAK / LinuxCNC-fork — Safety Boundary
...
> conformity advice.** It states where the boundary lies between the stmak
...
stmak is a **general-purpose machine-control software platform** (a LinuxCNC
...
NOT rely on stmak / LinuxCNC. They must be implemented in certified safety
hardware that is independent of the software and cannot be defeated by it.**

stmak is **not developed, verified, or rated to any functional-safety standard.**
...
Because stmak is a framework, the safety concept is necessarily machine-specific.
...
3. **Implement each safety function in certified safety hardware** to at least
   that level, independent of stmak — such that the function is achieved and
...
For every stmak software module, the assertion this document upholds is:
...
hardware. Design every safety function around the assumption that **stmak can and
...
crashed, wedged, or compromised stmak must not be able to keep the machine
...
| **EtherCAT master + lcec driver** (`cmd/ethercat`, master, driver cmods) | Cyclic exchange of process data with EtherCAT slaves; PDO/CoE config. | **Non-safe.** Ordinary process data carries no safety guarantee. **If Safety-over-EtherCAT (FSoE / TwinSAFE) is used, stmak is only a *black channel*:** it transports opaque safety telegrams; the safety function lives entirely in the certified FSoE master/slaves, and telegram loss/corruption/delay is detected by the FSoE watchdog + CRC, **not** by stmak. |
...
stmak's control surfaces — the **ADS server** (default TCP `48898`) and the
...
- an **external authenticated reverse proxy / API gateway** (authentication,
  TLS, and coarse allow/deny live outside stmak — see the security-model item in
...
1. stmak is **non-safety-rated** general-purpose software. Treat every output as
   non-safe.
2. **Operator-safety functions must be certified hardware, independent of
   stmak**, sized to a per-machine hazard analysis.

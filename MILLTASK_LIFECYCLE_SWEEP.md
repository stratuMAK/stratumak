...
reference (`~/source/linuxcnc-2.9`) diffed against `src/gomc/internal/task`,
...
- **stat.pocket_prepped** reports the pocket number, not the tooldata array
  index (F8) — gomc has no file-order table.
- **State-tag timing:** gomc tags a segment with the modal state *after* its
...
- **G64 blending extents** (abort/g64's remaining xfail): gomc's canon lacks

...
- Outlines drawn as separate `LINE`/`ARC` entities meeting end to end are
  chained into closed loops per layer (D30, 2026-09-08; `chain.go`): joined by
  endpoint coincidence within 1e-3 drawing units, any order and direction,
  arcs discretized circumscribed with tangents at their endpoints, then
  validated exactly like a polyline. An open chain, a branching junction, a
  segment shorter than the tolerance and an arc closing on itself are errors
  naming the coordinates. Chained dead zones (`ShapeSegments`) follow the
  single-entity ones in the scene, in the order of their first segment.
  Fixtures `testdata/chained_*.dxf` cover a rectangle of LINEs as outer
  limit, a stadium of two LINEs and two ARCs, shuffled/reversed segments, a
  mixed file and the three error cases.

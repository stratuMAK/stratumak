...
**Go modules can export RT functions (2026-08-17):** `pkg/hal` gained the last two rows a Go module was missing against a cmod — RT function export and the RT-hardened allocator (`NewRTComponent`, `ExportFunct`/`CFunct`, `RTCalloc`/`RTFree`, `Pin.RTDataPtr`; `docs/dev/GOMOD_RT_DESIGN.md`). The point is the *shape*: a module that needs one cyclic function no longer has to be written entirely in C, and the function it contains is still C, compiled from a real `.c` file with the same `stmak_hal.h`/`stmak_rt_check.h` a cmod uses. **There is deliberately no code path taking a Go `func`**, which turns §0's "no Go in the cycle" from an audited fact into an API property. `rt-effects-check.sh` section 2b now globs **every** `.c` under `src/stmak` and fails on one that is neither checked nor explicitly excused — so this scope cannot rot the way a hand-kept list does; it pulled in `classicladder_rt.c` (compiled, but its scan function is still unannotated, so nothing is verified about it yet) alongside halscope, the new `rtfuncttest` fixture and pnptask's `pnp_deadzone_rt.c`. **245 TUs green.** First customer: pnptask's `deadzone-check`, which reads the Cartesian feedback from `mot` — a new `@rt_safe` accessor `status_get_carte_pos_fb` reading motmod's RT status struct directly (no mutex, no full-status copy: the reader and the producer are the same thread), *not* from motstat, whose accessors take `reader_mtx` and could never honestly carry the annotation. The teardown question that made this risky is answered by the launcher as it already stands: it removes a component's functions and waits a full RT cycle (unload) or stops the threads synchronously (shutdown) before `Destroy`, so a module's whole obligation is to `RTFree` in `Destroy` and never in `Stop`.

**Biggest open gaps (priority order):**
1. **Effects-check scope**: remaining hand-written cmod TUs. classicladder's RT part is now *compiled* under the check but not yet *annotated* — `classicladder_refresh` needs `STMAK_NONBLOCKING` (and whatever that then diagnoses) to actually be verified. ~~The **EtherLab master library** needs its own audit session~~ — DONE 2026-07-20: the master now self-annotates and is verified transitively by section 8 of `rt-effects-check.sh` (see the follow-up above). RTSan (`-fsanitize=realtime`, needs a runtime harness against the sim) also still open. The motmod/tpmod/homemod/lcec TU lists in `rt-effects-check.sh` are still hand-copied from the Submakefiles (hostmot2 now globs) — a drift assertion against the Submakefile source variables would make list rot fail loudly.
...
- [x] RT cycle is 100% cgo-free and Go-pointer-free (no callback into Go)
  — True by construction: `thread_task()` walks the funct list and calls C function pointers only (`hal_lib.c:2982-2984`). Go-side modules that need an RT funct export a *C* function, never a Go trampoline, and all shared structs are C-allocated. Since 2026-08-17 this is also enforced at the API for Go modules: `pkg/hal`'s `ExportFunct` takes a `hal.CFunct` (a named type over `unsafe.Pointer`, populated from a symbol the module's `.c` file publishes) and there is no overload accepting a Go `func` — see `docs/dev/GOMOD_RT_DESIGN.md` §2/§4. Remaining caveat: nothing enforces it for hand-written *cmods*, which call `hal_export_funct` directly — that is the §2 tooling gap.

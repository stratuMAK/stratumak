...
For a headless machine, or to paste the state into a bug report, the same
...
`linuxcnctop-dump --json` prints the raw snapshot instead, and
`linuxcnctop-dump -n 1` repeats it once a second; `linuxcnctop-dump --help`
lists the rest.

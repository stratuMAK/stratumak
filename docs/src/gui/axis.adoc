...
* 'Follow Sub-files' - When execution enters an O-word subroutine held in a
  separate file, the interpreter numbers that file's lines from the start of
  it, so the running-line highlight cannot be shown against the loaded
  program. With this option on (the default) the program listing switches to
  the sub-file for the duration and a banner above it names the file being
  shown; it switches back when execution returns. With it off the listing
  stays on the loaded program and simply shows no highlight while execution
  is elsewhere. Either way, 'Run from here' applies only to the loaded
  program, so it is refused while a sub-file is on screen.
...
AXIS includes a program called 'linuxcnctop' which shows the details of
LinuxCNC's state. You can run this program by invoking Machine >
...
* a filter box, which matches both field names and values, so typing
  `estop` finds the field and typing `ESTOP` finds the machine state that
  holds it;
* the update rate;
* 'Freeze', which holds the display still so a fast-changing value can be
  read (the status bar says so while it is held);
* 'JSON', which shows the raw status snapshot;
* 'Copy All', which copies the current (filtered) view to the clipboard.

Lengths are shown in millimetres, which is the unit the controller keeps
internally, regardless of the machine's configured units.

From a terminal, run `linuxcnctop` to open the same window. For a headless
machine, or to paste the state into a bug report, add `-t` to print the
status instead:
...
`linuxcnctop --json` prints the raw snapshot instead, and `linuxcnctop -n 1`
repeats it once a second; `linuxcnctop --help` lists the rest. (The menu entry
runs `linuxcnctop-ui`, the window on its own.)
...
within a `~/.axisrc` Python script or a '[DISPLAY]USER_COMMAND_FILE'
Python script to make custom HAL pins. +
...
'axisui.abort-pin' pin will be 'TRUE' and come back to 'FALSE' after 0.3ms.

----
Type  Dir    Name
bit   OUT    axisui.abort-pin
----

AXIS has a HAL output pin that indicates when an error has occurred. The
'axisui.error-pin' pin will remain 'TRUE' until all error notifications have
been dismissed.

----
Type  Dir    Name
bit   OUT    axisui.error-pin
----

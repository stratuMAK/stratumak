...
// Plain listing rather than [source,{css}]: source-highlight escapes the <1>
// before asciidoc's callout substitution runs, so the callout below would not
// resolve.  Every other callout in these documents uses a plain listing too.
----
...
mess = {'NAME':'LOAD','ID':'_MY_DIALOG_',
            ...
            'FILENAME':'~/stratumak/nc_files/someprogram.txt',

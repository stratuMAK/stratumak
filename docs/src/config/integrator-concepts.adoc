...
* The LinuxCNC directory is located at '/home/user-name/stratumak'.
* The Configuration directories are located at '/home/user-name/stratumak/configs'.
**  Configuration files are located at '/home/user-name/stratumak/configs/name-of-config'.
* G-code files are located at /home/user-name/stratumak/nc_files'.

For example for a configuration called Mill and a user name Fred the directory
and file structure would look like this.

* '/home/fred/stratumak'
* '/home/fred/stratumak/nc_files'
* '/home/fred/stratumak/configs/mill'
** '/home/fred/stratumak/configs/mill/mill.ini'
** '/home/fred/stratumak/configs/mill/mill.hal'
** '/home/fred/stratumak/configs/mill/mill.var'
** '/home/fred/stratumak/configs/mill/tool.tbl'
...
options for running LinuxCNC from the command line run 'stmakd -h'.
...
LinuxCNC uses the RT_PREEMPT (also called PREEMPT_RT) realtime kernel
for real-time performance. The RT_PREEMPT patch modifies the Linux kernel
to provide realtime capabilities by making the kernel preemptible, allowing
high-priority realtime tasks to interrupt kernel code. This gives you the
ability to have things like software step generation which require precise
timing.

=== ACPI

The Advanced Configuration and Power Interface (ACPI) has a lot of
different functions, most of which can interfere with realtime performance
(for example: power management, CPU power down, CPU frequency scaling, etc).
For best realtime performance, it's recommended to disable features like
CPU frequency scaling in your BIOS or through kernel parameters.

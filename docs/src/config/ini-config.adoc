...
#INCLUDE ~/stratumak/myincludes/rs274ngc.inc
----
...
* `DEBUG = 0` - Debug level 0 means no messages will be printed when LinuxCNC is run from a <<faq:terminal,terminal>>.
  Debug flags are usually only useful to developers. See src/cnc/nml_intf/debugflags.h for other settings.
...
* `DISPLAY = axis` - The file name of the executable providing the user interface to use.
  Prominent valid options are (all in lower case): `axis`, `touchy`, `gmoccapy`, `gscreen`, `qtvcp`, `qtvcp-qtdragon` or `qtvcp-qtplasmac`.
...
* `MAX_SPINDLE_0_SPEED = 20000` - The maximum RPM that can be manually selected.
  On multi spindle machine there will be entries for each spindle number. Only used by the QtVCP-based user interfaces.
  - _deprecated_ - use the [SPINDLE_n] section instead.
* `PROGRAM_PREFIX = ~/stratumak/nc_files` - The default directory for G-code files, named subroutines, and user-defined M-codes.
...
* `OPEN_FILE = /full/path/to/file.ngc` - The program the controller opens at start up, so every
  connected UI shows the same one — including a UI that outlives a restart of the controller,
  which adopts whatever program the controller has open and never loads one of its own.
  A relative path is resolved against the program directories (`PROGRAM_PREFIX` and the other
  configured G-code locations), the same way a UI's File > Open is.
...
PROGRAM_EXTENSION = .gcode 3D Printer
  gcode = /home/mill/stratumak/convert.py
----
...
* `HOMEMOD =` _alternate_homing_module_ [home_parms=value]
  ...
  The setting may be overridden from the command line using the -m option ($ stmakd -h).
...
--
* `HALFILE = texample.tcl` - *No longer supported.* Tcl HAL files are rejected
  at startup with a hard error. The `haltcl` interpreter that processed them was
  removed together with the Tcl HAL binding; convert the file to plain `.hal`
  syntax.
...
* `APP =` _appname_ [arg1 [arg2 ...]]' - Application to be started.
  ...
  Examples:
** Simulate inputs to HAL pins for testing: load a pyvcp panel with widgets for
   the pins and connect them in a POSTGUI_HALFILE. See
   `configs/sim/axis/pin_control` for a ready-made example (it replaces the
   removed `sim_pin` utility).
...
* `TPMOD =` _alternate_trajectory_planning module_ [tp_parms=value] +
  ...
  The setting may be overridden from the command line using the -t option ($ stmakd -h).

...
[NOTE]
====
`moveoff_gui` has been *removed* -- it was a Tk panel on the deleted Tcl HAL
binding. Replacing it is config work only; the moveoff component already has
every pin required and no new component is needed:

* the two connections it made at startup (`mv.power-on` to
  `motion.motion-enabled`, `mv.apply-offsets` to `halui.program.is-paused`)
  belong in a HAL file;
* its `-autoresume -delay N` behaviour composes from `not`, `and2`, `timedelay`
  and `oneshot`;
* the display is covered by an existing pyvcp panel
  (`configs/sim/axis/moveoff/moveoff_display_7.xml`) or the gladevcp one.

See `configs/sim/axis/moveoff/README` for the exact connections and sequence.
The description below is kept because it documents what those connections have
to reproduce.
====

...
* 'motion.motion-type' - (s32, out) These values are from src/cnc/nml_intf/motion_types.h
  - 0: Idle (no motion)
  - 1: Traverse
  - 2: Linear feed
  - 3: Arc feed
...
* 'motion.queue-depth' -
  (s32, out) The number of motion segments the trajectory planner still has
  to run. Reaching zero while a move is in progress means the queue ran dry
  and the machine came to a stop; a task that streams segments ahead of the
  planner can use this to tell a genuine stop from a blend, which the
  velocity pins alone cannot show once the stop is shorter than the sampling
  interval.

...
After building the ladder, open the Project bar at the top of the page and save
it as estop.clp.

Now name that file on the module's load line in your custom.hal file, so the
ladder is running from start-up:

[source,{hal}]
----
# Load ClassicLadder and the ladder it runs
load classicladder estop.clp
----

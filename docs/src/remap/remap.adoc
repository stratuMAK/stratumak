...
$ mkdir python
$ cd python
$ cp /usr/share/stratumak/ncfiles/remap_lib/python-stdglue/stdglue.py .
...
The interpreter is an existing C++ class ('Interp') defined in 'src/cnc/rs274ngc'.
...
The actual canon functions are declared in `src/cnc/nml_intf/canon.hh` and implemented in `src/cnc/task/emccanon.cc`.
The implementation of the Python functions can be found in `src/cnc/rs274ncg/canonmodule.cc`.
...
`interpreter`::
  ...
  See `src/cnc/rs274ngc/interpmodule.cc`, and the `tests/remap/introspect` regression test.

`emccanon`::
  Exposes most calls of  `src/cnc/task/emccanon.cc`.

`emctask`::
  Exposes the `emcStatus` class instance. See `src/cnc/task/taskmodule.cc`.
...
m|EMC_DEBUG_PYTHON_TASK m|0x00040000 |trace the task Python plug in
...
For a current list of debug flags see 'src/cnc/nml_intf/debugflags.h'.
...
See 'Interp::convert_tool_select' in 'src/cnc/rs274/interp_execute.cc'.
...
Even if you are not familiar with C, I suggest you look at the 'Interp::convert_tool_change' code in 'src/cnc/rs274/interp_convert.cc'.
...
----
Debug level, 0 means no messages. See src/cnc/nml_intf/debugflags.h for others

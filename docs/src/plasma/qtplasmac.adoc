...
When the configuration is complete, the wizard will save a copy of the configuration that may be loaded and edited at a later time, a working QtPlasmaC configuration will be created in the following directory: ~/stratumak/configs/_<machine_name>_.
...
----
linuxcnc ~/stratumak/configs/_<machine_name>_/_<machine_name>_.ini
----

For a run in place installation: +
----
~/linuxcnc-dev/scripts/linuxcnc ~/stratumak/configs/_<machine_name>_/_<machine_name>_.ini
----
...
and create a new QtPlasmaC configuration directory in the ~/stratumak/configs directory.
...
Terminal=false
Name=LinuxCNC
Exec=sh -c "linuxcnc $HOME/stratumak/configs/<machine_name>/<machine_name>.ini"
Type=Application
...
. Select the Output File to write to, press *OUTPUT* to bring up a file selector or directly enter the file in the entry box.
  This would normally be ~/stratumak/configs/__<machine_name>___material.cfg.
...
[NOTE]
If the user selects ~/stratumak/configs/__<machine_name>___material.cfg and the file already exists, it will be overwritten.
...
Loading a G-code program from the directory specified by the *PROGRAM_PREFIX* variable in the _<machine_name>_.ini file (usually ~/stratumak/nc_files) is possible by using the following format:
...
[source,{ini}]
----
n Code = latest-file /home/me/stratumak/nc_files/qtplasmac-test
----
...
There are some sample G-code files in the ~/stratumak/nc_files/examples/plasmac directory.

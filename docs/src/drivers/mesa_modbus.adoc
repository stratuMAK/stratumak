...
----
./mesa-modcompile my_file.mod
----

and the "mesa-modcompile" script will compile and install the module. As
...
A simple script 'mesa-modcompile' is provided that will compile and install
...
necessary except in the special case of the 'mesa-modcompile all' command,
which will compile and install all .mod files in the current directory.

----
sudo mesa-modcompile my_file.mod
----

or

----
sudo mesa-modcompile all
----

"mesa-modcompile" is provided by the "stratumak-dev" package.
It is named "mesa-modcompile" to distinguish it from "modcompile", which compiles HAL components from .comp sources.

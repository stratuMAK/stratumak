...
For testing, it is convenient to simulate a jog wheel interface from a panel
that drives the eoffset pins (axis.z.eoffset-enable, -scale, -counts).
A pyvcp panel replaces the removed `sim_pin` utility for this -- see
`configs/sim/axis/pin_control` for a ready-made example.
...
----
[HAL]
# load a pyvcp panel with widgets for the eoffset pins, and connect them
# in a POSTGUI_HALFILE -- see configs/sim/axis/pin_control
HALFILE = pin_control_load.hal
POSTGUI_HALFILE = pin_control.hal
...
The panel provides controls for the axis offset pins: eoffset-scale

...
TWOPASS processing converted '.hal' files to equivalent '.tcl' files and used haltcl to find loadrt and addf commands in order to accumulate and consolidate their usage. NOTE: twopass is no longer implemented -- it went with the Tcl HAL binding, along with haltcl and halcmd_twopass.

...
'modcompile' will write all this code for you, automatically.
Compiling a HAL component is also much easier when using 'modcompile', whether the component is part of the LinuxCNC source tree, or outside it.

For instance, when coded in C, a simple component such as "ddt" is around 80 lines of code.
The equivalent component is very short when written using the 'modcompile' preprocessor:
...
'modcompile' can process, compile, and install a realtime component in a single step,
placing `rtexample.ko` in the LinuxCNC realtime module directory:

----
[sudo] modcompile --install rtexample.comp
----
...
'modcompile' can process, compile, install, and document non-realtime components.
A component is non-realtime because its `.comp` file says so, with 'option userspace yes'; there is no command-line switch for it.

----
modcompile non-rt-example.comp
modcompile --compile non-rt-example.comp
[sudo] modcompile --install non-rt-example.comp
modcompile --document non-rt-example.comp
----

'modcompile' compiles `.comp` files only.
To build a component written directly in C, drive the compiler from your own Makefile and ask 'modcompile' where things live:

----
modcompile --cflags        # compiler flags for cmod components
modcompile --ldflags       # linker flags
modcompile --include-dir   # cmod headers
modcompile --cmod-dir      # installation directory
modcompile --print-make-inc  # a ready-made Makefile include
----
...
More information on `load` and `addf` can be found in the
...
* *component* - A component is a single real-time module, which is loaded with `halcmd load`.
...
Instances are created when the component is loaded, and are named on the
'load' line: `load ddt <d1,d2,d3>` creates three. Classic comp's 'singleton',
'count=' and 'names=' schemes do not exist here — a component that would have
been a singleton is simply loaded once, and more instances are more names (or
further 'load' commands).
...
Remember that 'modcompile' interprets backslash escapes in strings, so for instance to set the italic font for the word 'example', write:
+
----
"\\fIexample\\fB"
----
...
* 'nofp' - Indicates that it only performs integer calculations.
  ...
  Neither 'modcompile' nor gcc can detect the use of floating-point calculations in functions that are tagged 'nofp',
  but the use of such operations results in undefined behavior.

=== Options

'modcompile' acts on these options:
...
* 'option extra_cleanup yes' - (default: no) +
  If specified, call the function defined by 'EXTRA_CLEANUP' when the instance is destroyed.

* 'option userspace yes' - (default: no) +
  If specified, this file describes a non-realtime component rather than a realtime one.
...
Any other option is an error, and 'modcompile' says so with the line that
caused it. An option that parsed but did nothing used to be accepted silently,
which moved the failure to load time — or to no time at all.

Three options from classic 'halcompile' are still accepted so that existing
components keep building, but each one warns that it does nothing:
'option fp' (floating point is a per-function flag: `function <name> fp;` /
`nofp`), 'option personality' (inferred from the pins that use it) and
'option default_personality' (pass `personality=N` on the 'load' line instead).

'option singleton' is rejected outright: cmod and gomod are always
multi-instance, and a second instance is a second 'load' command.

[[sec:comp-external-libraries]]
=== External libraries

A component that links something beyond the C library and libm declares it.
'modcompile' then compiles and links it directly — no external Makefile:

[source,c]
----
pkgconfig libcurl;                   // a pkg-config library
pkgconfig "libcurl >= 7.60.0";       // ...with a version constraint
pkgconfig libusb-1.0 zlib;           // several at once

cflags  "-DVENDOR_SDK -I/opt/vendor/include";   // a library with no .pc file
ldflags "-L/opt/vendor/lib -lvendor";
----

'pkgconfig' is the route to prefer. A bare name needs no quotes (the scanner
accepts '-' and '.', so 'libusb-1.0' is one name); a version constraint has to
be quoted, because of the operator. The flags are resolved by
`$PKG_CONFIG` — 'pkg-config' unless the environment says otherwise — so
'PKG_CONFIG_PATH', 'PKG_CONFIG_LIBDIR' and 'PKG_CONFIG_SYSROOT_DIR' apply as
they do to any other build, which is what makes a cross build work. Declaring
the dependency also makes it readable: `modcompile --deps foo.comp` prints the
module names, one per line, for a packager to turn into build dependencies.

'cflags' and 'ldflags' are the escape hatch for a library with no '.pc' file.
Their value is a literal string, split on whitespace and handed to the
compiler as separate arguments. There is deliberately no shell: `$(...)`,
backticks and `$VAR` are rejected rather than passed through, because
`modcompile --install` runs as root and a '.comp' has to stay data rather than
become a script.

Placement is 'modcompile''s problem, not the author's: compile flags go after
the built-in ones, so a module can override a default, and libraries go after
the object that references them.

When the flags can only come from running a program — 'llvm-config' and
friends — build the module from a Makefile instead: `modcompile --preprocess`
emits the C, and `modcompile --cflags` / `--ldflags` give you the rest. A
Makefile is expected to run programs.

NOTE: an external library belongs on a non-realtime entry point — a triggered
or threaded function, or an M-code handler. Calling one from a cyclic realtime
function is caught by the `-Wfunction-effects` realtime check
(see 'RT_HARDENING_CHECKLIST.md'), which is the point at which a blocking call
in the servo thread stops being your problem to notice.
...
Though HAL permits a pin, a parameter, and a function to have the same name, 'modcompile' does not.
...
Based on the items in the declaration section, 'modcompile' creates a C structure called `struct __comp_state`.
...
The details of `struct __comp_state` and these macros may change from one version of 'modcompile' to the next.
...
'Personality' is given per instance when the module is loaded, and instance storage is allocated at that point, so there is no compile-time ceiling on the number of items and no switch to raise one.
Hexadecimal forms work as well as decimal, for example `personality=0x102`.

When using components with personality, each instance is given its
personality on its own 'load' line.
Example for 3 instances of the logic component:

[source,{hal}]
----
load logic <and4>  personality=0x104
load logic <or3>   personality=0x203
load logic <nand5> personality=0x805
----

An instance loaded without a personality gets a personality of 0.
...
The pin is called 'out_' because 'out' is an identifier used by the I/O macros.
It illustrates a per-instance module parameter, and 'EXTRA_SETUP' / 'EXTRA_CLEANUP' bringing the hardware to a known state and putting it back.

One instance per card, each given its address on its own 'load' line — that is
what replaces the classic 'count' parameter and 'get_count()':
...
startup, for which it needs the 'cap_sys_rawio' capability, so every instance
can already reach the port; 'rtapi_request_region' survives only as a no-op for
source compatibility. If that capability is missing the port access faults, as
it does for any other port-I/O driver in the tree.
...
Typical load lines for this component might be

[source,{hal}]
----
load logic <logic.0> personality=0x102
load logic <logic.1> personality=0x305
load logic <logic.2> personality=0x503
----

which create the following pins:
...
The modcompile man page gives details for invoking `modcompile`:

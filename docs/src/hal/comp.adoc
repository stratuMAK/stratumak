...
* 'option extra_link_args "..."' - (default: "")
   ...
   NOTE: 'modcompile' does not act on this option; use `modcompile --ldflags` from your own Makefile instead.
...
* 'option extra_compile_args "..."' - (default: "")
   ...
   NOTE: 'modcompile' does not act on this option; use `modcompile --cflags` from your own Makefile instead.
...
'modcompile' prints its own usage, which is the authoritative list of options:

----
$ modcompile --help
----

NOTE: The installed `modcompile(1)` man page describes a different, unrelated utility for compiling Modbus drivers for Mesa PktUART cards, not the component compiler used here.

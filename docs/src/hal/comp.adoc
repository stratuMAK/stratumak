...
NOTE: this example is carried over from classic 'halcompile' and has not been
ported. 'option count_function' and 'option constructable' do not exist in
'modcompile' and are rejected, the instance count comes from the names on the
'load' line instead, and 'EXTRA_SETUP' takes no 'extra_arg'. It is kept for the
shape of 'EXTRA_SETUP'/'EXTRA_CLEANUP', not as something to copy.

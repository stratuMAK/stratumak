...
** latency-test - standard test (runs the realtime threads and opens the
   web-based latency viewer with live summary, plot and histogram)
...
for a configuration to a new subdirectory of ~/stratumak/configs
(equivalently: /home/username/stratumak/configs).  Each
...
----
cd ~/stratumak/configs/name_of_configuration

...
1. **Effects-check scope**: hostmot2 and remaining hand-written cmod TUs are not yet annotated/checked. The **EtherLab master library** needs its own audit session (its RT interface is currently trusted via `ecrt_rt_api.h`). RTSan (`-fsanitize=realtime`, needs a runtime harness against the sim) also still open.

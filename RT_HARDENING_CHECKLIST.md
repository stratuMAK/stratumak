...
1. **Effects-check scope**: hostmot2, lcec (EtherCAT) and remaining hand-written cmod TUs are not yet annotated/checked — extend `scripts/rt-effects-check.sh` TU by TU (tpmod/homemod implementations now carry the obligation via the annotated vtable types). RTSan (`-fsanitize=realtime`, needs a runtime harness against the sim) also still open.

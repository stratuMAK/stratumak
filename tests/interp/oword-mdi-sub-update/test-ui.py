...
# Where test.sh's `gomc_start_server interp.ini` (no --log/--inherit) puts the
...
c = gomc_test.Command(); s = gmi.Stat()
...
gomc_test.wait_file_contains(SERVER_LOG, 'test RAN', count=1)
...
if os.system('./edit.sh') != 0:
    gomc_test.fail("edit.sh failed to rewrite subs/test1.ngc")
c.mdi('o<test1> call [20]'); c.wait_complete()
gomc_test.wait_file_contains(SERVER_LOG, 'test RAN', count=2)

c.mdi('o<test1> call [20]'); c.wait_complete()
gomc_test.wait_file_contains(SERVER_LOG, 'test RAN', count=3)

#!/usr/bin/env python3
"""Self-test for lib/python/gomc_test.py.

The whole suite now synchronises through gomc_test, so a bug in its waiters
...
def test_returns_predicate_value_immediately():
    ...
    got = gomc_test.wait_until(lambda: "value", "an already-true condition")
...
def test_raises_on_deadline_and_names_what_it_waited_for():
    try:
        gomc_test.wait_until(lambda: False, "the thing that never happens",
                             timeout=0.1)
...
def test_detail_is_included_in_the_failure():
    try:
        gomc_test.wait_until(lambda: False, "a condition", timeout=0.1,
...
def test_scale_stretches_the_deadline():
    ...
    try:
        ...
        del os.environ["GOMC_TEST_TIMEOUT_SCALE"]


def test_scale_ignores_garbage():
    os.environ["GOMC_TEST_TIMEOUT_SCALE"] = "not-a-number"
    try:
        expect(gomc_test.scale() == 1.0, "a malformed scale must fall back to 1.0")
    finally:
        del os.environ["GOMC_TEST_TIMEOUT_SCALE"]
...
def test_wait_file_contains_sees_a_late_write():
    with tempfile.TemporaryDirectory() as d:
        ...
        t.start()
        try:
            body = gomc_test.wait_file_contains(path, "test RAN", timeout=10)
...
def test_wait_file_contains_honours_count():
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(path, "w") as f:
            f.write("hit\n")
        try:
            gomc_test.wait_file_contains(path, "hit", timeout=0.2, count=2)
        except gomc_test.Timeout:
            return
...
def test_wait_file_stable_waits_for_the_writer_to_finish():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        t.start()
        try:
            body = gomc_test.wait_file_stable(path, timeout=10, settle=0.1)
...
def test_missing_file_times_out_rather_than_raising_oserror():
    try:
        gomc_test.wait_file_contains("/nonexistent/nope", "x", timeout=0.1)
    except gomc_test.Timeout:
        return
...
print("gomc_test helper self-test: all checks passed")

...
#   stmak_rel     path (relative to this dir) of the stmak gold to compare
...
#                config; 1 = mm config). stmak golds are always mm (factor 1).
#
# Only parity-able tests are listed. Excluded and why:
#   motion-logger/startup-gcode-abort — enabled and passing with a stmak gold, but
#                                        not a comparable oracle: 2.9 defers the
#                                        startup move via interp_list (0 SET_LINE),
#                                        stmak dispatches it at estop (1 SET_LINE).

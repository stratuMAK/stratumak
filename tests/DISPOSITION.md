# Runtests disposition ledger (stratuMAK)

One central, honest record of every test in `tests/` that does **not** run-and-pass
green against stratuMAK, and every `expected` oracle re-baselined from classic-C to stratuMAK output.
...
## 1. Re-baselined `expected` (oracle: classic-C → stratuMAK)

### 1a. Benign — format-only (stratuMAK `halcmd`/loader output shape; no behavioral divergence)
...
| loadrt.2 | line-split + stratuMAK single-instance naming `streamer.0.*`→`streamer.*` |
...
These tests exercised features stratuMAK removed for good (embedded Python interpreter,
...
| remap/fail/body-py | `REMAP=M400 py=interp_error` — a pure Python remap **body** returning INTERP_ERROR | stratuMAK rejects `py=`/`python=` at parse. Handler-fails-conveys-error is covered by remap/fail/{prolog,epilog}; only the py= body form is gone. |
...
convention. stratuMAK's UIs are REST/WS web apps; there is no wish-like emcsh and the
...
| rtapi_printf.0 | custom `rtapi_vsnprintf` %f formatter | 🗑 **DELETED** — custom kernel-safe `rtapi_vsnprintf` removed (stratuMAK uspace-only → libc); no meaningful re-expression. rtapi_print/rtapi_print_msg remain but formatting is libc's now. (Was a precise skip; removed 2026-07-13 as a genuine feature removal.) |
...
| overrun | runtests overrun-*retry* workaround | ⛔ **DROPPED** — it tested `run_without_overruns` (re-run a `test.hal` up to 10× if it prints `overrun`), a flakiness-masking retry that was dormant in stratuMAK (nothing emits `overrun`) and is a workaround, not behavior. Removed `run_without_overruns` from `runtests.in` (a `.hal` now runs once); deleted the test. Tests must be deterministic, not retried. |
| halmodule.0 | pin type/range coercion | ✅ **PASS** — ported: haljson creates s32/u32/float pins, gmi client POSTs values over REST (`haljson.writePin` range-coerces, matching the classic oracle line-for-line). Found+fixed a real stratuMAK bug: haljson nil-INI deref under `-f` (mirrors the pyvcp fix). Binding-object introspection (is_pin/getitem) dropped — that's the removed userspace-Python-binding API. |
...
The stratuMAK replacement for embedded-Python interpreter extensions is the C interp_ext API
...
**stratuMAK bug fixed here (interp error conveyance):** a C interp_ext prolog/epilog/O-word handler that called `ctx->set_error()` and
returned INTERP_EXT_ERROR had its saved message clobbered with a generic "pycall(...) failed" / "handler not registered".
Root cause: stratuMAK's `Interp::pycall` returned the handler's mapped status directly, tripping the caller's
...
### 3a. Resolved — the 8 xfails from the 07-15 snapshot (was: "runnable, fail on a documented stratuMAK bug")

Every entry is now green or removed. Note two of the five originally-blamed "stratuMAK bugs" turned
...
|---|---|---|
| gmi.Stat client field gaps | startup-state, mdi-queue-length | ✅ **fixed.** startup-state: stratuMAK now exposes the full boot status surface (optional_stop/block_delete default ON, spindle brake engaged at init) + an opt-in `Stat.machine_units()` inch view (645c941773, 2c0137aedc). mdi-queue-length: milltask now tracks MDI queue depth and gmi.Stat exposes `queued_mdi_commands` — it was XPASSing (cc06432180). |
...
mid-travel by stratuMAK's INTENDED 2 s jog dead-man watchdog (`task.go jogTimeout`,
...
personality arrays, `--personalities` count-cycling, userspace `count=`/`names=` instancing. stratuMAK has
...
| test | action |
|---|---|
| threads.0 · threads.1 | ✅ **PASS** — ported: `newthread` fast/slow + `threadtest` counter, captured by `filestream` (stratuMAK has no userspace `halsampler`). threads.0 verifies the 10:1 period-ratio scheduling over 3500 samples; threads.1 verifies per-function `tmax` is nonzero (read via `show param` — stratuMAK `getp` doesn't resolve RW params, noted in PRODUCTION_READINESS). |
| module-loading/rtapi-app-main-fails | ✅ **PASS** — ported to the cmod/`load` model: a comp fails its init via a failing `EXTRA_SETUP` (`-ERANGE`), and `load` correctly fails (`factory returned error code`). Classic used `option rtapi_app no` + custom `rtapi_app_main`. Note: stratuMAK flattens the errno to `-1` (documented gap). |
| mdi-queue/simple-queue-buster | ✅ **PASS** — ported to REST/gmi (via `rsh2gmi.py`, M100 captured by `mcode_coord_log format=p`, a new P-only mode). Bulk MDI interleaving `t1 m6`/`t2 m6` tool-change queue-busters with `m100 p<i>`; all 1001 M100s logged in order. stratuMAK's `mdi()` is synchronous so the queue-buster serialises rather than races, but the tool-change-vs-MDI sequencing is exercised. |
...
| test | action |
|---|---|
| trajectory-planner/circular-arcs | ✅ **PASS** — added an automated regression test (the classic dir was a developer profiling/tuning harness — operf/octave/interactive — with no `test.sh` in either branch). New `test.sh`/`checkresult`/`arc.ngc`/`arc.ini`/`capture.hal`/`run-arc.py`: run a full-circle G3 through the stratuMAK TP, capture the commanded X/Y joint path every servo cycle with `filestream`, and assert every sample lies on the commanded circle (centre (0,10), r=10) within 0.05 mm and that the arc swept the whole circle. stratuMAK traces it with 0.00000 mm deviation. The legacy profiling files are kept as dev tooling (see `README.stmak`). |

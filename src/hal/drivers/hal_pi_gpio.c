...
static int number_of_cores(const stmak_log_t *log)
{
    char str[256];
    int procCount = 0;
    FILE *fp;
        ...
        int argc, const char **argv, cmod_t **out)
{
    ...
    stmak_log_infof(inst->log, "hal_pi_gpio", "%d cores rev %d", ncores, rev);
	...
	stmak_log_infof(inst->log, "hal_pi_gpio", "The Pi model %i is not known to "
	...
	if ((retval = stmak_hal_pin_bit_newf(hal, STMAK_HAL_IN, &inst->port_data[n],
				       inst->comp_id, "hal_pi_gpio.pin-%02d-out", pinno)) < 0)
	  break;
      } else {
	bcm2835_gpio_fsel(inst, inst->gpios[n], BCM2835_GPIO_FSEL_INPT);
	if ((retval = stmak_hal_pin_bit_newf(hal, STMAK_HAL_OUT, &inst->port_data[n],
    ...
    stmak_log_infof(inst->log, "hal_pi_gpio",

...
// FIXME: this is bogus
static void hm2_7i90_nanosleep(const stmak_rtapi_t *rtapi, unsigned long int nanoseconds) {
...
int hm2_7i90_read(hm2_lowlevel_io_t *this, uint32_t addr, void *buffer, int size) STMAK_NONBLOCKING {
...
int hm2_7i90_write(hm2_lowlevel_io_t *this, uint32_t addr, const void *buffer, int size) STMAK_NONBLOCKING {
    int bytes_remaining = size;
    hm2_7i90_t *board = this->private;
...
int hm2_7i90_reset(hm2_lowlevel_io_t *this) {
    ...
    hm2_7i90_nanosleep(inst->env->rtapi, 1000);
    ...
    hm2_7i90_nanosleep(inst->env->rtapi, 2 * 1000 * 1000);
        ...
        int argc, const char **argv, cmod_t **out)
{
    ...
    hm2_7i90_inst_t *inst = env->rtapi->calloc(env->rtapi->ctx, sizeof(*inst));
    ...
    if (!inst->core) {
        stmak_logf(env->log, name, STMAK_LOG_ERROR | STMAK_LOG_OPER, "hm2_7i90: hostmot2 core API not found (is hostmot2 loaded?)\n");

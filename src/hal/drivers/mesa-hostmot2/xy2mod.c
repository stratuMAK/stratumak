...
void hm2_xy2mod_process_tram_read(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
static void hm2_xy2mod_instance_write(hostmot2_t *hm2, int i) STMAK_NONBLOCKING {
    double stepsx_cmd;
    double stepsy_cmd;
...
static void hm2_xy2mod_set_dpll_rtimer(hostmot2_t *hm2) STMAK_NONBLOCKING {
    uint32_t data = 0;
...
static void hm2_xy2mod_set_dpll_wtimer(hostmot2_t *hm2) STMAK_NONBLOCKING {
    uint32_t data = 0;
...
void hm2_xy2mod_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    for (i = 0; i < hm2->xy2mod.num_instances; i ++) {
...
void hm2_xy2mod_force_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
...
int hm2_xy2mod_parse_md(hostmot2_t *hm2, int md_index) {
...

    {
        ...
        if (hm2->dpll_module_present) {
            ...
            r = stmak_hal_pin_s32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.hal->pin.dpll_rtimer_num), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
        ...
        if (hm2->dpll_module_present) {
            ...
            r = stmak_hal_pin_s32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.hal->pin.dpll_wtimer_num), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
        ...
        for (i = 0; i < hm2->xy2mod.num_instances; i ++) {
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.posx_cmd), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.posy_cmd), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.velx_cmd), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.vely_cmd), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.accx_cmd), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.accy_cmd), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.velx_fb), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.vely_fb), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.posx_fb), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.posy_fb), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.posx_scale), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.posy_scale), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
            ...
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.enable), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.controlx", hm2->llio->name, i);
            r = stmak_hal_pin_u32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.controlx), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.controly", hm2->llio->name, i);
            r = stmak_hal_pin_u32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.controly), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.commandx", hm2->llio->name, i);
            r = stmak_hal_pin_u32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.commandx), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.commandy", hm2->llio->name, i);
            r = stmak_hal_pin_u32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.commandy), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.18bitmodex", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.mode18bitx), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.18bitmodey", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.mode18bity), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.commandmodex", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.commandmodex), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.commandmodey", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->xy2mod.instance[i].hal.pin.commandmodey), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.posx-overflow", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.posx_overflow), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.posy-overflow", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.posy_overflow), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.velx-overflow", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.velx_overflow), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.vely-overflow", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.vely_overflow), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
           ...
           snprintf(name, sizeof(name), "%s.xy2mod.%02d.status", hm2->llio->name, i);
            r = stmak_hal_pin_u32_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->xy2mod.instance[i].hal.pin.status), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                r = -ENOMEM;
                goto fail0;
...
void hm2_xy2mod_print_module(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    if (hm2->xy2mod.num_instances <= 0) return;

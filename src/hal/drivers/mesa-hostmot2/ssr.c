...
int hm2_ssr_parse_md(hostmot2_t *hm2, int md_index) {
...

    {
        ...
        for (i = 0; i < hm2->ssr.num_instances; i ++) {
            ...
            r = stmak_hal_pin_u32_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->ssr.instance[i].hal.pin.rate), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
...

            {
                ...
                for (j = 0; j < hm2->num_pins; j++){
                    if (hm2->pin[j].sec_tag == HM2_GTAG_SSR && hm2->pin[j].sec_unit == i) {
                        ...
                        r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->ssr.instance[i].hal.pin.out[ssr_number]), hm2->llio->comp_id, "%s", name);
                        if (r < 0) {
                            HM2_ERR("error adding pin '%s', aborting\n", name);
                            r = -ENOMEM;
                            goto fail1;
                        ...
                        r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->ssr.instance[i].hal.pin.invert[ssr_number]), hm2->llio->comp_id, "%s", name);
                        if (r < 0) {
                            HM2_ERR("error adding pin '%s', aborting\n", name);
                            r = -ENOMEM;
                            goto fail1;
...
void hm2_ssr_force_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int size;
    int i;
...
void hm2_ssr_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
void hm2_ssr_prepare_tram_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
void hm2_ssr_print_module(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    if (hm2->ssr.num_instances <= 0) return;

...
void hm2_pwmgen_force_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    uint32_t pwm_width;
...
void hm2_pwmgen_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
int hm2_pwmgen_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        for (i = 0; i < hm2->pwmgen.num_instances; i ++) {
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->pwmgen.instance[i].hal.pin.value), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
            ...
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->pwmgen.instance[i].hal.pin.enable), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail1;
            }
            ...
            r = stmak_hal_param_bit_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->pwmgen.instance[i].hal.param.offset_mode), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
            ...
            if (hm2->pwmgen.firmware_supports_dither) {
                ...
                r = stmak_hal_param_bit_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->pwmgen.instance[i].hal.param.dither), hm2->llio->comp_id, "%s", name);
                if (r < 0) {
                     HM2_ERR("error adding param '%s', aborting\n", name);
                    goto fail1;
                }
            ...
            r = stmak_hal_param_float_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->pwmgen.instance[i].hal.param.scale), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail1;
            }

            r = stmak_hal_param_s32_newf(hm2->llio->hal, 
                STMAK_HAL_RW,
                &(hm2->pwmgen.instance[i].hal.param.output_type),
                hm2->llio->comp_id,
...
void hm2_pwmgen_print_module(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    if (hm2->pwmgen.num_instances <= 0) return;
...
void hm2_pwmgen_prepare_tram_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    double topdrop;
    if (hm2->pwmgen.num_instances <= 0) return;

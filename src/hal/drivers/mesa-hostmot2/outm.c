        ...
        for (i = 0; i < hm2->outm.num_instances; i ++) {
            ...
            for (j = 0; j < hm2->num_pins; j++){
                if (hm2->pin[j].sec_tag == HM2_GTAG_OUTM && hm2->pin[j].sec_unit == i) {
                    ...
                    r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->outm.instance[i].hal.pin.out[outm_number]), hm2->llio->comp_id, "%s", name);
                    if (r < 0) {
                    ...
                    r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->outm.instance[i].hal.pin.invert[outm_number]), hm2->llio->comp_id, "%s", name);
                    if (r < 0) {
                        HM2_ERR("error adding pin '%s', aborting\n", name);
                        r = -ENOMEM;
                        goto fail0;
...
void hm2_outm_force_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int size;
    int i;
...
void hm2_outm_prepare_tram_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
void hm2_outm_print_module(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    if (hm2->outm.num_instances <= 0) return;

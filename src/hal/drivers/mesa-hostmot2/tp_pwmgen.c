...
void hm2_tp_pwmgen_force_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
void hm2_tp_pwmgen_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
...
//
void hm2_tp_pwmgen_queue_read(hostmot2_t *hm2) STMAK_NONBLOCKING {
...
void hm2_tp_pwmgen_process_read(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    for (i = 0 ; i < hm2->tp_pwmgen.num_instances ; i++) {
...
int hm2_tp_pwmgen_parse_md(hostmot2_t *hm2, int md_index) {
    ...
    hm2->tp_pwmgen.setup_reg = (uint32_t *)hm2->llio->rtapi->calloc(hm2->llio->rtapi->ctx, hm2->tp_pwmgen.num_instances * sizeof(uint32_t));
    if (hm2->tp_pwmgen.setup_reg == NULL) {
        HM2_ERR("out of memory!\n");
        r = -ENOMEM;
        goto fail1;
    }
    hm2->tp_pwmgen.enable_reg = (uint32_t *)hm2->llio->rtapi->calloc(hm2->llio->rtapi->ctx, hm2->tp_pwmgen.num_instances * sizeof(uint32_t));
    ...
    // FIXME: r hides the r in enclosing function, and it returns the wrong thing
    {
        ...
        for (i = 0; i < hm2->tp_pwmgen.num_instances; i ++) {
            ...
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->tp_pwmgen.instance[i].hal.pin.Avalue), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
	    ...
	    snprintf(name, sizeof(name), "%s.3pwmgen.%02d.B-value", hm2->llio->name, i);
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->tp_pwmgen.instance[i].hal.pin.Bvalue), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            }
	    ...
	    snprintf(name, sizeof(name), "%s.3pwmgen.%02d.C-value", hm2->llio->name, i);
            r = stmak_hal_pin_float_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->tp_pwmgen.instance[i].hal.pin.Cvalue), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            }
            ...
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_IN, &(hm2->tp_pwmgen.instance[i].hal.pin.enable), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            }
	    ...
	    snprintf(name, sizeof(name), "%s.3pwmgen.%02d.fault", hm2->llio->name, i);
            r = stmak_hal_pin_bit_newf(hm2->llio->hal, STMAK_HAL_OUT, &(hm2->tp_pwmgen.instance[i].hal.pin.fault), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding pin '%s', aborting\n", name);
                goto fail2;
            }
            ...
            r = stmak_hal_param_float_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->tp_pwmgen.instance[i].hal.param.scale), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
	    ...
	    snprintf(name, sizeof(name), "%s.3pwmgen.%02d.deadtime", hm2->llio->name, i);
            r = stmak_hal_param_float_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->tp_pwmgen.instance[i].hal.param.deadzone), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail2;
            }
	    ...
	    snprintf(name, sizeof(name), "%s.3pwmgen.%02d.fault-invert", hm2->llio->name, i);
            r = stmak_hal_param_bit_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->tp_pwmgen.instance[i].hal.param.faultpolarity), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail2;
            }
	    ...
	    snprintf(name, sizeof(name), "%s.3pwmgen.%02d.sample-time", hm2->llio->name, i);
            r = stmak_hal_param_float_newf(hm2->llio->hal, STMAK_HAL_RW, &(hm2->tp_pwmgen.instance[i].hal.param.sampletime), hm2->llio->comp_id, "%s", name);
            if (r < 0) {
                HM2_ERR("error adding param '%s', aborting\n", name);
                goto fail2;
            }
...
fail2:
    hm2->llio->rtapi->free(hm2->llio->rtapi->ctx, hm2->tp_pwmgen.enable_reg);

fail1:
    hm2->llio->rtapi->free(hm2->llio->rtapi->ctx, hm2->tp_pwmgen.setup_reg);
...
void hm2_tp_pwmgen_print_module(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;
    if (hm2->tp_pwmgen.num_instances <= 0) return;
...
void hm2_tp_pwmgen_prepare_tram_write(hostmot2_t *hm2) STMAK_NONBLOCKING {
    int i;

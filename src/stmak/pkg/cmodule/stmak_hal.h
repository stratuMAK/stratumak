// stmak_hal.h — HAL component/pin/param/function API for stratuMAK C modules.
//
// The numeric constants in this header (types, directions, name lengths)
// mirror hal.h by John Kasunich. License of hal.h: LGPL Version 2.
// Copyright (c) 2003 John Kasunich.
//
// New API (callback table, inline helpers):
// Copyright (C) 2026 Sascha Ittner <sascha.ittner@modusoft.de>
// License: LGPL Version 2
//
// All HAL operations go through callbacks in stmak_hal_t, which the launcher
// populates at module load time.  These callbacks delegate to
// liblinuxcnchal.so.
//
// Typed pin/param creation and printf-style name formatting are provided as
// inline convenience functions in this header — the callbacks themselves use
// generic (type-as-int) signatures to keep the vtable small.
//
// Usage:
//   int comp_id = env->hal->init(env->hal->ctx, name, env->dl_handle,
//                                STMAK_HAL_COMP_REALTIME);
//   stmak_hal_pin_bit_newf(env->hal, STMAK_HAL_OUT, &my_pin, comp_id,
//                         "%s.enable", name);
//   env->hal->ready(env->hal->ctx, comp_id);

#ifndef STMAK_HAL_H
#define STMAK_HAL_H

#include <stdarg.h>
#include <stdio.h>
#include <stdint.h>

#ifdef __cplusplus
extern "C" {
#endif

#include "stmak_rt_check.h"

// Signature of a HAL realtime (cyclic) function.  The nonblocking type
// annotation makes registering an unannotated function via export_funct
// a -Wfunction-effects diagnostic under "make rt-effects-check".
// Duplicated deliberately from hal_funct_ptr_t in src/hal/hal.h (this
// header set is self-contained for external modules) — the two meet at
// the export_funct seam in launcher/cmodules.go, keep them in sync.
typedef void (*stmak_hal_funct_t)(void *arg, long period) STMAK_NONBLOCKING;

// ---------------------------------------------------------------------------
// HAL type / direction / component-type constants.
//
// These mirror the values in hal.h so that modules using the new API don't
// need to include the old header.  The numeric values MUST match hal.h.
// ---------------------------------------------------------------------------

typedef enum {
    STMAK_HAL_TYPE_UNSPECIFIED = -1,
    STMAK_HAL_BIT   = 1,
    STMAK_HAL_FLOAT = 2,
    STMAK_HAL_S32   = 3,
    STMAK_HAL_U32   = 4,
    STMAK_HAL_PORT  = 5,
} stmak_hal_type_t;

typedef enum {
    STMAK_HAL_DIR_UNSPECIFIED = -1,
    STMAK_HAL_IN  = 16,
    STMAK_HAL_OUT = 32,
    STMAK_HAL_IO  = (16 | 32),
} stmak_hal_pin_dir_t;

typedef enum {
    STMAK_HAL_RO = 64,
    STMAK_HAL_RW = (64 | 128),
} stmak_hal_param_dir_t;

typedef enum {
    STMAK_HAL_COMP_USER     = 0,
    STMAK_HAL_COMP_REALTIME = 1,
} stmak_hal_comp_type_t;

// HAL data types — these must match hal.h typedefs.
typedef volatile int32_t      stmak_hal_s32_t;
typedef volatile uint32_t     stmak_hal_u32_t;
typedef volatile double       stmak_hal_float_t;
typedef volatile unsigned     stmak_hal_bit_t;
typedef volatile unsigned     stmak_hal_port_t;

// Maximum HAL name length (matches HAL_NAME_LEN in hal.h).
#define STMAK_HAL_NAME_LEN 127

// ---------------------------------------------------------------------------
// stmak_hal_t — HAL callback table.
// ---------------------------------------------------------------------------

typedef struct {
    void *ctx;

    // Component lifecycle.
    int   (*init)  (void *ctx, const char *name, void *dl_handle, int type);
    void  (*exit)  (void *ctx, int comp_id);
    int   (*ready) (void *ctx, int comp_id);

    // HAL shared-memory allocation (mlock'd, page-faulted).
    void *(*malloc)(void *ctx, long size);

    // Pin and parameter creation (generic, type passed as int).
    int   (*pin_new)  (void *ctx, const char *name, int type, int dir,
                       void **data_ptr_addr, int comp_id);
    int   (*param_new)(void *ctx, const char *name, int type, int dir,
                       void *data_addr, int comp_id);

    // Pin/param aliasing.
    int   (*pin_alias)  (void *ctx, const char *pin_name, const char *alias);
    int   (*param_alias)(void *ctx, const char *param_name, const char *alias);

    // RT function export.
    int   (*export_funct)(void *ctx, const char *name,
                          stmak_hal_funct_t funct,
                          void *arg, int uses_fp, int reentrant, int comp_id);
} stmak_hal_t;

// ---------------------------------------------------------------------------
// Convenience: generic pin_new with printf-style name.
// ---------------------------------------------------------------------------

static inline __attribute__((format(printf, 6, 7))) int
stmak_hal_pin_newf(const stmak_hal_t *hal, int type, int dir,
                  void **data_ptr_addr, int comp_id,
                  const char *fmt, ...) __attribute__((unused));
static inline int
stmak_hal_pin_newf(const stmak_hal_t *hal, int type, int dir,
                  void **data_ptr_addr, int comp_id,
                  const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->pin_new(hal->ctx, name, type, dir, data_ptr_addr, comp_id);
}

// ---------------------------------------------------------------------------
// Typed pin creation — type-safe wrappers with format checking.
// ---------------------------------------------------------------------------

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_pin_bit_newf(const stmak_hal_t *hal, stmak_hal_pin_dir_t dir,
                      stmak_hal_bit_t **data_ptr_addr, int comp_id,
                      const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->pin_new(hal->ctx, name, STMAK_HAL_BIT, dir,
                        (void **)data_ptr_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_pin_float_newf(const stmak_hal_t *hal, stmak_hal_pin_dir_t dir,
                        stmak_hal_float_t **data_ptr_addr, int comp_id,
                        const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->pin_new(hal->ctx, name, STMAK_HAL_FLOAT, dir,
                        (void **)data_ptr_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_pin_u32_newf(const stmak_hal_t *hal, stmak_hal_pin_dir_t dir,
                      stmak_hal_u32_t **data_ptr_addr, int comp_id,
                      const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->pin_new(hal->ctx, name, STMAK_HAL_U32, dir,
                        (void **)data_ptr_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_pin_s32_newf(const stmak_hal_t *hal, stmak_hal_pin_dir_t dir,
                      stmak_hal_s32_t **data_ptr_addr, int comp_id,
                      const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->pin_new(hal->ctx, name, STMAK_HAL_S32, dir,
                        (void **)data_ptr_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_pin_port_newf(const stmak_hal_t *hal, stmak_hal_pin_dir_t dir,
                       stmak_hal_port_t **data_ptr_addr, int comp_id,
                       const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->pin_new(hal->ctx, name, STMAK_HAL_PORT, dir,
                        (void **)data_ptr_addr, comp_id);
}

// ---------------------------------------------------------------------------
// Convenience: generic param_new with printf-style name.
// ---------------------------------------------------------------------------

static inline __attribute__((format(printf, 6, 7))) int
stmak_hal_param_newf(const stmak_hal_t *hal, int type, int dir,
                    void *data_addr, int comp_id,
                    const char *fmt, ...) __attribute__((unused));
static inline int
stmak_hal_param_newf(const stmak_hal_t *hal, int type, int dir,
                    void *data_addr, int comp_id,
                    const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->param_new(hal->ctx, name, type, dir, data_addr, comp_id);
}

// ---------------------------------------------------------------------------
// Typed param creation — type-safe wrappers with format checking.
// ---------------------------------------------------------------------------

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_param_bit_newf(const stmak_hal_t *hal, stmak_hal_param_dir_t dir,
                        stmak_hal_bit_t *data_addr, int comp_id,
                        const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->param_new(hal->ctx, name, STMAK_HAL_BIT, dir,
                          (void *)data_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_param_float_newf(const stmak_hal_t *hal, stmak_hal_param_dir_t dir,
                          stmak_hal_float_t *data_addr, int comp_id,
                          const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->param_new(hal->ctx, name, STMAK_HAL_FLOAT, dir,
                          (void *)data_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_param_u32_newf(const stmak_hal_t *hal, stmak_hal_param_dir_t dir,
                        stmak_hal_u32_t *data_addr, int comp_id,
                        const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->param_new(hal->ctx, name, STMAK_HAL_U32, dir,
                          (void *)data_addr, comp_id);
}

static inline __attribute__((format(printf, 5, 6))) int
stmak_hal_param_s32_newf(const stmak_hal_t *hal, stmak_hal_param_dir_t dir,
                        stmak_hal_s32_t *data_addr, int comp_id,
                        const char *fmt, ...) {
    char name[STMAK_HAL_NAME_LEN + 1];
    va_list ap;
    va_start(ap, fmt);
    vsnprintf(name, sizeof(name), fmt, ap);
    va_end(ap);
    return hal->param_new(hal->ctx, name, STMAK_HAL_S32, dir,
                          (void *)data_addr, comp_id);
}

#ifdef __cplusplus
}
#endif

#endif // STMAK_HAL_H

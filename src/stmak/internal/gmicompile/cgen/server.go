...
func (g *serverGen) emitHeaderGuard() {
	...
	g.printf("// Generated by modcompile gmi - DO NOT EDIT\n")
	EmitMeta(g.w, g.api, "//")
	g.printf("#ifndef %s\n", guard)
	g.printf("#define %s\n\n", guard)
}
...
func (g *serverGen) emitIncludes() {
	...
	// callback types (mirrors STMAK_NONBLOCKING in stmak_rt_check.h; the
	// generated header must not depend on the cmodule header set).
	g.printf("\n#ifndef STMAK_API_NONBLOCKING\n")
	g.printf("#if defined(__clang__) && (__clang_major__ >= 20) && defined(__has_attribute)\n")
	g.printf("#if __has_attribute(nonblocking)\n")
	g.printf("#define STMAK_API_NONBLOCKING __attribute__((nonblocking))\n")
	g.printf("#endif\n")
	g.printf("#endif\n")
	g.printf("#ifndef STMAK_API_NONBLOCKING\n")
	g.printf("#define STMAK_API_NONBLOCKING\n")
	g.printf("#endif\n")
	g.printf("#endif\n")
...
func (g *serverGen) emitCallbackDecls() {
	...
	for _, cb := range g.api.Callbacks {
		...
		if cb.RTSafe {
			g.printf(") STMAK_API_NONBLOCKING;\n\n")
		} else {
			g.printf(");\n\n")
		}
	}
...
func (g *serverGen) emitRegistration() {
	...
	g.printf("//\n")
	g.printf("// These static inline wrappers call through the stmak_api_t callback\n")
	...
	g.printf("#ifndef %s\n\n", guard)
	g.printf("#include \"stmak_api.h\"\n\n")

	// Register wrapper
	g.printf("static inline int %s_api_register(\n", name)
	g.printf("    const stmak_api_t *api,\n")

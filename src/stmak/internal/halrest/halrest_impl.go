...
import (
	...
	halcmdapi "github.com/stratuMAK/stratumak/src/stmak/generated/gmi/halcmd"
...
func (h *halcmdImpl) Load(module string, args []string) (*halcmdapi.CmdResult, error) {
	if loadModuleHook == nil {
		return errCmd(fmt.Errorf("load: not supported (stmakd launcher not initialized)"))
	}
...
func (h *halcmdImpl) Unload(name string) (*halcmdapi.CmdResult, error) {
	if unloadModuleHook == nil {
		return errCmd(fmt.Errorf("unload: not supported (stmakd launcher not initialized)"))
	}

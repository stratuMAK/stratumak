...
func init() {
	stmak.RegisterModule("halscope", newHalscope)
	apiserver.RegisterMeta(halscopeapi.HalscopeMeta)
}

// halscope implements stmak.Module.
...
func newHalscope(ini *inifile.IniFile, logger *slog.Logger, name string, args []string) (stmak.Module, error) {
	...
	// Create HAL RT component.  We need the stmakd binary's own

...
struct haldata {
    stmak_hal_float_t *pfr, *tl, *sl, *fr;
};
        ...
        int argc, const char **argv, cmod_t **out)
{
    ...
    int rc;
    rc = stmak_hal_pin_float_newf(env->hal, STMAK_HAL_IN, &haldata->pfr,
                                 g_comp_id, "%s.platformradius", name);
    if (rc < 0) goto fail;
    rc = stmak_hal_pin_float_newf(env->hal, STMAK_HAL_IN, &haldata->tl,
                                 g_comp_id, "%s.thighlength", name);
    if (rc < 0) goto fail;
    rc = stmak_hal_pin_float_newf(env->hal, STMAK_HAL_IN, &haldata->sl,
                                 g_comp_id, "%s.shinlength", name);
    if (rc < 0) goto fail;
    rc = stmak_hal_pin_float_newf(env->hal, STMAK_HAL_IN, &haldata->fr,

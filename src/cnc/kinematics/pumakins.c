...
struct haldata {
    stmak_hal_float_t *a2, *a3, *d3, *d4, *d6;
};
        ...
        int argc, const char **argv, cmod_t **out)
{
    ...
    int rc = 0;
    rc |= stmak_hal_pin_float_newf(env->hal, STMAK_HAL_IN, &haldata->a2,
                                  g_comp_id, "%s.A2", name);
    rc |= stmak_hal_pin_float_newf(env->hal, STMAK_HAL_IN, &haldata->a3,
                                  g_comp_id, "%s.A3", name);

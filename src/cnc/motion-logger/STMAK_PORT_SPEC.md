...
independent stmak/gmi feature gaps (NOT interceptor issues):
...
- **ctx struct:** `{ FILE *log; const stmak_api_t *api; char mot_inst_name[];
...
- **Build:** mirror `src/cnc/motion/Submakefile`'s motmod cmod rule (smaller — needs
...
`New`/`Init`/`Start`/`Destroy` mirror `src/cnc/motion/motion.c`, but in Init it:
...
`src/cnc/motion-logger/motion-logger.c` byte-for-byte, e.g.
`SET_LINE x=%.6g, y=%.6g, …, id=%d, motion_type=%d, vel=%.6g, ini_maxvel=%.6g, acc=%.6g, turn=%d\n`
(the provider callbacks carry all the args — see `src/cnc/motion/motctl_handlers.c`).
...
**Build:** add `cmod/motion-logger.so` mirroring `src/cnc/motion/Submakefile`'s
...
3. Only after (2), delete `motcmd_trace()` + the `#ifdef MILLTASK_PARITY_TRACE`
   block + its call site from `src/cnc/motion/command.c` — motmod then carries no

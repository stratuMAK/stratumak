 ...
 */
// gomc_path.h — configuration path resolution for gomc C modules.
//
...
//                                         GOMC_PATH_READ, &err);
//   if (!path) {
//       gomc_log_errorf(env->log, name, "config file %s: %s", filename, err);
//       goto fail;
//   }
//   FILE *f = fopen(path, "r");
...
// gomc_path_t — path resolution callbacks.
//
...
typedef struct {
    void *ctx;
    const char* (*resolve)(void *ctx, const char *name, gomc_path_mode_t mode,
                           const char **err_out);
} gomc_path_t;

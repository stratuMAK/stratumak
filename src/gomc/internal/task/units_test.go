...
func TestLoadJoint_InchLinearScaledToMM(t *testing.T) {
...
`)
	...
	rec := newRec()
	if err := loadJoint(ini, task, 0, rec); err != nil {
		t.Fatalf("loadJoint: %v", err)
	}
	if !task.jointLinear[0] {
...
func TestLoadJoint_InchAngularNotScaled(t *testing.T) {
...
`)
	...
	task := &Task{linearUnits: 1.0 / inch}
	rec := newRec()
	if err := loadJoint(ini, task, 3, rec); err != nil {
		t.Fatalf("loadJoint: %v", err)
	}
	if task.jointLinear[3] {
...
func TestLoadAxis_LinearVsAngular(t *testing.T) {
...
`)
	...
	task := &Task{linearUnits: 1.0 / inch}
	rec := newRec()
	if err := loadAxis(ini, task, 2, rec); err != nil { // Z
		t.Fatalf("loadAxis Z: %v", err)
	}
	if err := loadAxis(ini, task, 3, rec); err != nil { // A
		t.Fatalf("loadAxis A: %v", err)
	}
	// Z (linear) scaled
...
`)
	...
	if err := loadTraj(ini, task2, newRec()); err != nil {

...
// GOMC_API_NONBLOCKING iff the callback is @rt_safe — mirroring the @rt_safe/_fn
...
func TestCallbackRTSafeAnnotation(t *testing.T) {
	...
	if !rtSafeTypedefAnnotated(out, "rttest_tick_fn_cb") {
		t.Errorf("rt_safe callback tick_fn should close with GOMC_API_NONBLOCKING:\n%s", out)
	}
...
// declaration with GOMC_API_NONBLOCKING (i.e. ") GOMC_API_NONBLOCKING;" appears
...
func rtSafeTypedefAnnotated(out, cbName string) bool {
	...
	return strings.Contains(rest[:end], "GOMC_API_NONBLOCKING")
}

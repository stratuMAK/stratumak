...
// $STATE/bin/gomc-server is a server somebody rebuilt, a symlink is the
// package's own.
func stalenessFixture(t *testing.T, builtVersion, installedVersion string, localBuild bool) string {
	...
	sources := filepath.Join(base, "usr", "gomc")

	oldState, oldGomc, oldVer := config.EMC2StateDir, config.EMC2GomcDir, config.EMC2Version
	t.Cleanup(func() {
		config.EMC2StateDir, config.EMC2GomcDir, config.EMC2Version = oldState, oldGomc, oldVer
	})
	config.EMC2StateDir = state
	config.EMC2GomcDir = sources
	...
	if installedVersion != "" {
		p := filepath.Join(sources, gomcVersionFile)
		if err := os.WriteFile(p, []byte(installedVersion+"\n"), 0o644); err != nil {
			t.Fatalf("writing %s: %v", p, err)
		}
	}

	server := filepath.Join(state, "bin", "gomc-server")
	if localBuild {
		if err := os.WriteFile(server, []byte("ELF"), 0o755); err != nil {
			t.Fatalf("writing %s: %v", server, err)
		}
	} else if err := os.Symlink("/usr/libexec/stratumak/gomc-server", server); err != nil {
...
// server rebuilt against a previous release's gomc, still running after the

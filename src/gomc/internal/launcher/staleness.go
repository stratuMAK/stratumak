...
// gomcVersionFile is the version stamp installed alongside the gomc sources.
...
const gomcVersionFile = "VERSION"

// WarnIfStale reports a locally rebuilt gomc-server that no longer matches the
// installed gomc sources.
//
...
func WarnIfStale(logger *slog.Logger) {
	...
	fi, err := os.Lstat(filepath.Join(localBin, "gomc-server"))
	if err != nil || fi.Mode()&os.ModeSymlink != 0 {
		return
	}

	data, err := os.ReadFile(filepath.Join(config.GomcDir(), gomcVersionFile))
	if err != nil {
	...
	logger.Warn("this gomc-server was built locally from older sources; run `sudo modcompile rebuild`",
		"server_built_from", config.EMC2Version,
		"installed_sources", installed,
		"sources", config.GomcDir())
}

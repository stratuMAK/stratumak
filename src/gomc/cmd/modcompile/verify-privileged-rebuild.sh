...
#
#   sudo src/gomc/cmd/modcompile/verify-privileged-rebuild.sh
#
...
#
# It builds gomc-server three times, so allow a quarter of an hour. Nothing outside
...
LIBEXEC=/usr/libexec/stratumak/gomc-server
...
for d in "$STATE" "$STATE/bin" "$STATE/cmod" "$STATE/modules" "$STATE/gomc"; do
...
[ -d "$CACHE/tree/gomc" ] || fail "$CACHE/tree/gomc missing; the scratch copy must be named gomc
      so that a \"gomc/generated/...\" include resolves"
pass "scratch tree is named gomc"

# ---------------------------------------------------------------------------
step "3. the installed result"
# ---------------------------------------------------------------------------

server="$STATE/bin/gomc-server"

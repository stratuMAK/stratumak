...
- [ ] **Security model / API authentication** (ruling 2026-07-21, see
  ...
  **New surface, 2026-07-29:** the controller now **executes a user-configured program** — the
  ...
  gomc-server's file capabilities (Linux recomputes capabilities from the executed file; nothing
...
- [ ] **Deployment/rollback** — version stamp visible in UI/logs, defined update procedure,
  ...
  external-module install work (see the status log): the installed gomc sources carry a
  ...
  controller is one command, `ln -sf /usr/libexec/stratumak/gomc-server
  /var/lib/stratumak/bin/gomc-server`, because the packaged binary is never overwritten.
...
needs either a small fix or a ruling. Ask-before-fix applies.
...
15. **milltask [LOW]** — an unknown `load milltask` arg is silently ignored
    (would have caught the no-op `db=pnp-tools.db` in CoatV2conf); make it a
    hard error.
...
19. **programfilter [LOW]** — a `/tmp/gomc-filtered-<pid>` directory survives a
    SIGKILL (removal happens at shutdown). **A startup sweep was proposed and
    ruled out** (user, 2026-07-29): pid reuse plus concurrent instances means a
...
- **Unit tests** — all under `make gomc-test` (so: PR gate + nightly race build). There is no

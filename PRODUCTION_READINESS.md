...
failure** (the C callback returns the struct by value with no `rc` out-param, so **13** client
methods — persist 8, tooltable 4, `emcio.GetStatus` — are structurally unable to signal an error).
**The REST surface is unaffected** (its handler calls the Go provider directly); the blind
consumers are the in-process ones, Go *and* C — incl. `ioControl_v2.c`'s 5 `tt->get_tool` sites on
the tool-change path. **The fix mechanism already exists and is in production:** an `out` param
plus an `i32` return (`motstat.gmi`'s `get_status(status: MotionStatus out) -> i32`), and it
leaves the Go client signature *identical*, so Go consumers compile unchanged. Prerequisite is
**G-2**, a second finding surfaced while scoping this: the dispatch emitter treats an `out` param
as an *input*, reading it from the request and discarding the value the provider filled in
(latent — every IDL using `out` today is `@rest_export false`). Same class as the "Surface RCS
command errors to clients" cross-cutting item; needs a ruling before generator work starts. Note this contradicts the

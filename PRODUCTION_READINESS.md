...
exists for, unnoticed because the bare entry point worked. The only Phase-5 item the pass did not
cover is `internal/inirest` (57.5 %). The REST/WS surface still has **no authentication** (model is
...
scope and stays as it is. emccalib's `U` stays ◐: `GetTunables`/`SaveIni` read live HAL pins

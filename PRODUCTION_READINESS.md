...
| Date | Event |
|---|---|
| 2026-07-23 | **Webapp CI job added — the webapps are now a merge gate.** They previously had *no* CI presence at all (not built/type-checked/tested/linted in `ci.yml`); `make webapp-lint`/`webapp-test` were local-only. New `webapps` job in `.github/workflows/ci.yml` runs on every push and PR (like `static-checks` — self-contained, npm only, no RIP build) and calls `scripts/check-webapps.sh`: ESLint (shared config) + cold `vue-tsc -b --force` type-check on all five non-deferred apps + vitest on the four with tests. The script is the CI source of truth (app lists kept in sync with `webapp/Submakefile`), shellcheck-clean and added to `check-static`'s lint list. Local run green: eslint clean, 5/5 type-check, tests 34+19+18+14. So the webapp `L`/`U`/tests are now enforced by CI, not just local runs. |

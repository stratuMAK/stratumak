...
itself, and recorded here rather than papered over): the process-lifetime entry
points (`RtapiAppInit`/`Cleanup`, `UnloadAll`, the mlock helpers) would tear down
the test binary's own HAL; and `cmd/modcompile` stays at ~10 % because the rest
of it shells out to a C compiler or rewrites the source tree — the build
regenerates and compiles every shipped `.comp` on each run, which is the stronger
signal.

...
1. **Runtests against gomc** — DONE (2026-07-13; updated 2026-07-15 after the lifecycle sweep):
   full suite green, 242 run / 220 pass / 0 fail / 22 xfail / 0 skipped;
...
- **G64 blending extents — canon gap.** gomc's canon does not implement 2.9's naive CAM detector (`emccanon.cc` chord-deviation segment merging for `G64 Qx`), and the G64 / G64 P<tol> corner-rounding motion extents diverge from 2.9 (corners stay exact-stop-sharp). Belongs with the M67 canon-queue-contract workstream. (tests/abort/g64 extent checks)

...
**`U` gaps CLOSED 2026-07-22 — every Phase-4 row is now `U` ✅** (see the status
...
assertions. Coverage: halcmd 9.5→60.8 %, halparse 72→87 %, haljson 18.6→93.1 %,
modcompile 59.8→86.1 % (cross-package), cmd/halcmd 1.7→78.1 %.
**Residual, deliberately not unit-tested** (covered by runtests / the build
itself, and recorded here rather than papered over): HAL *thread* creation needs
RT scheduling privileges, so `TestThreadLifecycle`/`TestAddFDelFErrors` skip when
`rtapi_is_realtime()` is false (they run on an RT-capable machine); the
process-lifetime entry points (`RtapiAppInit`/`Cleanup`, `UnloadAll`, the mlock
helpers) would tear down the test binary's own HAL; and `cmd/modcompile` stays at
~10 % because the rest of it shells out to a C compiler or rewrites the source
tree — the build regenerates and compiles every shipped `.comp` on each run,

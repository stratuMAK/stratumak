...
**Needs a full runtests round** — a `[SEC]KEY` that fails to resolve now errors
(exactly 2.9's behavior). Still flagged (not auto-fixable): HC-2 (ambiguous
arg-path heuristic), HJ-2 (drain contract — ties to ADS A5 / pkg-hal H1), HF-1
(`LIB:` scope), HP-6/HP-7 (`$(VAR)` syntax / dropped `getp`·`print` output).
`U` ◐ where happy-path coverage still wanted.

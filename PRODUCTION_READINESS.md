...
1. **Runtests against gomc** — DONE (2026-07-13; updated 2026-07-15 after the lifecycle sweep
   and the sync-I/O/stale-status un-xfail sweep): full suite green,
   242 run / 223 pass / 0 fail / 19 xfail / 0 skipped;
...
- **G64 blending extents — canon gap.** gomc's canon does not implement 2.9's naive CAM detector at all: `G64 Qx` is parsed into `canon.go` `naivecamTol` (and served by the getter) but nothing consumes it — 2.9's `chained_points`/`see_segment`/`flush_segments` machinery (`emccanon.cc:875-1030`: buffer consecutive G1s while continuous+Q>0, ABCUVW constant, ≤100 pts, chord deviation ≤ Q; merged move carries the LAST point's line number + state tag) has no gomc equivalent. Also the G64 / G64 P<tol> corner-rounding motion extents diverge from 2.9 (corners stay exact-stop-sharp) — verify plain G64 P term-cond actually reaches the TP before attributing all of it to the missing Q-merger. Sequencing: do this DIRECTLY AFTER the M67 sync-I/O fix — in 2.9 `SET_MOTION_OUTPUT_BIT`'s first act is `flush_segments()`, i.e. the segment buffer and the syncdio attach share one emission-ordering contract; porting the buffer without that flush discipline would re-break M62-M68. The port itself is ~150 lines; the real work is the flush-site sweep over every gomc canon entry point that emits or reads endPoint. (tests/abort/g64 extent checks)

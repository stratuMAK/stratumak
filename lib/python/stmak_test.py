...
2. **Never call `gmi.Command.wait_complete()` bare.** A wait that did not happen
   ...
   the useful message is the server's, not urllib's. `stmak_test.Command` (below)
   translates it into a `Timeout` naming the deadline.

Timeouts are deliberately generous. Set STMAK_TEST_TIMEOUT_SCALE=3 to stretch
...
Usage::
    ...
    import stmak_test
    stmak_test.install_constants()      # linuxcnc.STATE_ON etc.

    c = stmak_test.Command()            # strict wait_complete
    s = stmak_test.Stat()
    stmak_test.wait_for_startup(s)

    c.mdi("M61 Q3")
    c.wait_complete()
    stmak_test.wait_stat(s, lambda st: st.tool_in_spindle == 3,
                        "tool 3 in spindle")
"""
...
_SCALE_ENV = "STMAK_TEST_TIMEOUT_SCALE"
...
def scale() -> float:
    """Multiplier applied to every deadline (STMAK_TEST_TIMEOUT_SCALE)."""
    try:
...
def wait_until(pred, desc, timeout=None, interval=DEFAULT_INTERVAL,
    ...
    Arguments:
        ...
        timeout  — seconds, before STMAK_TEST_TIMEOUT_SCALE. None = DEFAULT_TIMEOUT.
